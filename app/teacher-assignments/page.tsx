...
try {
  ...
  /*
   * Load the main school data first.
   *
   * IMPORTANT:
   * Terms are NOT queried using school_id.
   * They are connected to academic_year_id.
   */
  const [
    teachersResult,
    yearsResult,
  ...
  /*
   * Select the current academic year automatically.
   */
  const currentYear =
    loadedYears.find((year) => year.is_current === true) ||
    loadedYears[0];
  ...
  /*
   * Load terms/semesters through academic_year_id.
   */
  if (loadedYears.length > 0) {
    ...
    /*
     * Automatically select the current semester
...
/*
* First try the normal school-scoped query.
*/
const { data, error: assignmentError } =
await supabase
.from('teacher_assignments')
.select("id, teacher_id, class_id, subject_id, term_id, teacher:users!teacher_assignments_teacher_id_fkey ( id, full_name, email ), class:classes ( id, name, level, academic_year_id, programme_id ), subject:subjects ( id, name, code ), term:terms ( id, name, academic_year_id, is_current )")
.eq('school_id', schoolId)
...
* Only show semesters belonging to the
...
* 
* 1. Remove an invalid class selection.
* 2. Remove an invalid semester selection.
* 3. Automatically select the current semester
* for that academic year when available.
  */
  useEffect(() => {
  if (!selectedYear) {
  setSelectedClass('');
...
const selectedTermRecord = terms.find(
  (term) => term.id === selectedTerm
);

if (
  selectedTermRecord &&
  selectedTermRecord.academic_year_id !==
    selectedYear
) {
  setError(
    'The selected semester belongs to a different academic year.'
  );
  return;
}
...
try {
  ...
  /*
   * Use the existing teacher_assignments table.
   * No new assignment system is created.
   */
