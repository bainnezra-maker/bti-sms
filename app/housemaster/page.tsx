...
export default async function HousemasterDashboardPage() {
  ...
  const [boarders, houses, rooms, capacities, allocations, checkouts, exiats, incidents, recent] =
    await Promise.all([
      supabase.from('students').select('id',{count:'exact'}).eq('school_id',schoolId).eq('resident','Boarding').eq('status','active'),
  ...
  const boarderIds=new Set((boarders.data||[]).map(x=>x.id));
