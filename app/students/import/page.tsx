...
type ImportRow={admission_number:string;full_name:string;form:string;programme:string;class_name:string;gender:string;resident:string;house:string;date_of_birth:string;region:string;place_of_birth:string;religion:string;bece_index_number:string;father_name:string;father_contact:string;mother_name:string;mother_contact:string;residential_address:string;digital_address:string;whatsapp_number:string;admission_date:string;jhs_aggregate:string;health_issues:string;health_insurance_number:string;health_insurance_expiry_date:string};
...
const headers=['STUDENT ID','FULL NAME','FORM','PROGRAMME','CLASS','GENDER','RESIDENCE','HOUSE','DATE OF BIRTH','REGION','PLACE OF BIRTH','RELIGION','BECE INDEX NUMBER','FATHER NAME','FATHER CONTACT','MOTHER NAME','MOTHER CONTACT','RESIDENTIAL ADDRESS','DIGITAL ADDRESS','WHATSAPP NUMBER','ADMISSION DATE','JHS AGGREGATE','HEALTH ISSUES','HEALTH INSURANCE NUMBER','HEALTH INSURANCE EXPIRY DATE'];
...
const normalizeHouse=(v:unknown)=>clean(v);
...
function mapRow(row:Record<string,unknown>):ImportRow{const get=(n:string)=>{const k=Object.keys(row).find(x=>normalize(x)===normalize(n));return k?row[k]:''};return{admission_number:clean(get('STUDENT ID')),full_name:clean(get('FULL NAME')),form:clean(get('FORM')),programme:clean(get('PROGRAMME')),class_name:clean(get('CLASS')),gender:clean(get('GENDER')),resident:clean(get('RESIDENCE')),house:clean(get('HOUSE')),date_of_birth:excelDateToString(get('DATE OF BIRTH')),region:clean(get('REGION')),place_of_birth:clean(get('PLACE OF BIRTH')),religion:clean(get('RELIGION')),bece_index_number:clean(get('BECE INDEX NUMBER')),father_name:clean(get('FATHER NAME')),father_contact:clean(get('FATHER CONTACT')),mother_name:clean(get('MOTHER NAME')),mother_contact:clean(get('MOTHER CONTACT')),residential_address:clean(get('RESIDENTIAL ADDRESS')),digital_address:clean(get('DIGITAL ADDRESS')),whatsapp_number:clean(get('WHATSAPP NUMBER')),admission_date:excelDateToString(get('ADMISSION DATE')),jhs_aggregate:clean(get('JHS AGGREGATE')),health_issues:clean(get('HEALTH ISSUES')),health_insurance_number:clean(get('HEALTH INSURANCE NUMBER')),health_insurance_expiry_date:excelDateToString(get('HEALTH INSURANCE EXPIRY DATE'))}}
...
export default function StudentImportPage(){
 ...
 function downloadTemplate(){
  const sample=[{'STUDENT ID':'9030301-2026-119','FULL NAME':'John Mensah',FORM:'Form 1',PROGRAMME:'Electrical Engineering',CLASS:'A Class',GENDER:'Male',RESIDENCE:'Boarding',HOUSE:'House 1','DATE OF BIRTH':'2010-05-12',REGION:'Greater Accra','PLACE OF BIRTH':'Accra',RELIGION:'Christianity','BECE INDEX NUMBER':'0123456789','FATHER NAME':'Kwame Mensah','FATHER CONTACT':'0240000000','MOTHER NAME':'Ama Mensah','MOTHER CONTACT':'0550000000','RESIDENTIAL ADDRESS':'Accra','DIGITAL ADDRESS':'GA-123-4567','WHATSAPP NUMBER':'0240000000','ADMISSION DATE':'2026-09-01','JHS AGGREGATE':'18','HEALTH ISSUES':'','HEALTH INSURANCE NUMBER':'NHIS123456789','HEALTH INSURANCE EXPIRY DATE':'2027-08-31'}];
 ...
 async function importStudents(){
  ...
  rows.forEach((r,i)=>{const row=i+2,res=normalizeResidence(r.resident),house=normalizeHouse(r.house),studentId=clean(r.admission_number);if(!studentId){results.push({row,status:'skipped',type:'warning',message:'Skipped before import: STUDENT ID is required.'});return}if(!STUDENT_ID_REGEX.test(studentId)){results.push({row,status:'skipped',type:'warning',message:'Skipped before import: STUDENT ID must follow the format 9030301-2026-119.'});return}if(!clean(r.full_name)){results.push({row,status:'skipped',type:'warning',message:'Skipped before import: FULL NAME is required.'});return}if(res==='Boarding'&&!house){results.push({row,status:'skipped',type:'warning',message:'Skipped before import: HOUSE is required for Boarding students.'});return}if(r.jhs_aggregate&&!Number.isFinite(Number(r.jhs_aggregate.replace(/,/g,'')))){results.push({row,status:'skipped',type:'warning',message:'Skipped before import: JHS AGGREGATE must be a valid number.'});return}valid.push({...r,admission_number:studentId,full_name:clean(r.full_name),resident:res,house:res==='Boarding'?house:'',row_number:row})});
  ...
  valid.forEach(r=>{const fp=normalize(r.admission_number);const first=fingerprints.get(fp);if(first)results.push({row:r.row_number,status:'skipped',type:'warning',message:`Skipped before import: STUDENT ID ${r.admission_number} is repeated in Excel row ${first.row_number}.`});else{fingerprints.set(fp,r);send.push(r)}});
 ...
 return <main className="min-h-screen bg-slate-50 p-4 pt-20 sm:p-6 sm:pt-20 lg:p-8">
  <div className="mx-auto max-w-7xl">
   ...
   <section className="mt-6 overflow-hidden rounded-3xl border border-slate-200 bg-white shadow-sm">
    ...
    <div className="mt-5 rounded-2xl border border-purple-100 bg-purple-50 p-4 text-sm text-purple-900"><div className="flex gap-3"><i className="fa-solid fa-house-user mt-0.5 text-purple-600"/><div><p className="font-black">Boarding House & Health Insurance</p><p className="mt-1 text-xs leading-5 text-purple-700">STUDENT ID is required for every row and must follow the format 9030301-2026-119. For Boarding students, HOUSE is required and may contain the school's actual house name. Other profile and health fields may be left blank when details are not yet available.</p></div></div></div>
   ...
   {rows.length>0&&<section className="mt-6 overflow-hidden rounded-3xl border border-slate-200 bg-white shadow-sm animate-[fadeIn_0.4s_ease-out]">
    <div className="p-5 md:p-6"><div className="flex flex-col gap-4 md:flex-row md:items-center md:justify-between"><div><h2 className="text-xl font-black text-slate-900">Student Preview</h2><p className="mt-1 text-sm text-slate-500">{rows.length} record(s) loaded. Review before importing.</p></div><button onClick={importStudents} disabled={importing||!academicYearId} className="inline-flex items-center justify-center gap-2 rounded-2xl bg-blue-600 px-5 py-3.5 text-sm font-black text-white shadow-sm transition duration-300 hover:-translate-y-1 hover:bg-blue-700 hover:shadow-xl disabled:cursor-not-allowed disabled:opacity-50"><i className={importing?'fa-solid fa-spinner fa-spin':'fa-solid fa-cloud-arrow-up'}/>{importing?'Processing...':'Import & Assign Students'}</button></div>
     <div className="mt-5 overflow-x-auto rounded-2xl border border-slate-200 shadow-sm"><table className="min-w-[3400px] text-left text-xs"><thead className="bg-slate-100"><tr>{headers.map(h=><th key={h} className={`whitespace-nowrap px-3 py-3.5 font-black ${h==='RESIDENCE'?'bg-blue-50 text-blue-800':h==='HOUSE'?'bg-purple-50 text-purple-800':'text-slate-700'}`}><div className="flex items-center gap-2">{h}{h==='RESIDENCE'&&<i className="fa-solid fa-house-user text-blue-600"/>}{h==='HOUSE'&&<i className="fa-solid fa-building text-purple-600"/>}</div></th>)}</tr></thead><tbody>{rows.slice(0,50).map((r,i)=><tr key={i} className="border-t border-slate-200 transition duration-200 hover:bg-blue-50/40">
      <td className="whitespace-nowrap px-3 py-3 font-black text-blue-700">{r.admission_number||'—'}</td>
      <td className="whitespace-nowrap px-3 py-3 font-bold text-slate-800">{r.full_name||'—'}</td>
      <td className="whitespace-nowrap px-3 py-3">{r.form||'—'}</td>
      <td className="whitespace-nowrap px-3 py-3">{r.programme||'—'}</td>
      <td className="whitespace-nowrap px-3 py-3">{r.class_name||'—'}</td>
      ...
      <td className="whitespace-nowrap bg-purple-50/30 px-3 py-3 font-bold text-purple-800">{r.house||'—'}</td>
      <td className="whitespace-nowrap px-3 py-3">{r.date_of_birth||'—'}</td>
      <td className="whitespace-nowrap px-3 py-3">{r.region||'—'}</td>
      <td className="whitespace-nowrap px-3 py-3">{r.place_of_birth||'—'}</td>
      <td className="whitespace-nowrap px-3 py-3">{r.religion||'—'}</td>
      <td className="whitespace-nowrap px-3 py-3">{r.bece_index_number||'—'}</td>
      <td className="whitespace-nowrap px-3 py-3">{r.father_name||'—'}</td>
      <td className="whitespace-nowrap px-3 py-3">{r.father_contact||'—'}</td>
      <td className="whitespace-nowrap px-3 py-3">{r.mother_name||'—'}</td>
      <td className="whitespace-nowrap px-3 py-3">{r.mother_contact||'—'}</td>
      <td className="min-w-[220px] px-3 py-3">{r.residential_address||'—'}</td>
      <td className="whitespace-nowrap px-3 py-3">{r.digital_address||'—'}</td>
      <td className="whitespace-nowrap px-3 py-3">{r.whatsapp_number||'—'}</td>
      <td className="whitespace-nowrap px-3 py-3">{r.admission_date||'—'}</td>
      <td className="whitespace-nowrap px-3 py-3">{r.jhs_aggregate||'—'}</td>
      <td className="min-w-[220px] px-3 py-3">{r.health_issues||'—'}</td>
      <td className="whitespace-nowrap px-3 py-3">{r.health_insurance_number||'—'}</td>
      <td className="whitespace-nowrap px-3 py-3">{r.health_insurance_expiry_date||'—'}</td>
