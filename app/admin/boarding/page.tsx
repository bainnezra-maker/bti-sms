...
export default async function AdminBoardingDashboard() {
  ...
  return <main className="min-h-screen bg-slate-50 px-4 py-6 sm:px-6 lg:px-8"><link rel="stylesheet" href="https://cdnjs.cloudflare.com/ajax/libs/font-awesome/6.7.2/css/all.min.css"/><div className="mx-auto max-w-7xl space-y-6">
    ...
    <section className="overflow-hidden rounded-3xl border bg-white shadow-sm">
      <div className="border-b p-5"><p className="text-xs font-black uppercase tracking-widest text-slate-400">Administrator report · Read only</p><h2 className="mt-1 text-lg font-black"><i className="fa-solid fa-bed mr-2"/>Room &amp; Bed Allocation Report</h2><p className="mt-1 text-xs text-slate-500">Every active boarding allocation, grouped by house and room. Only Housemasters can change these assignments.</p></div>
      {allocations.length ? <div className="overflow-x-auto"><table className="w-full min-w-[720px] text-left text-sm"><thead className="bg-slate-50 text-[11px] font-black uppercase tracking-wider text-slate-500"><tr><th className="px-5 py-3">Student</th><th className="px-5 py-3">Admission No.</th><th className="px-5 py-3">House</th><th className="px-5 py-3">Room</th><th className="px-5 py-3">Bed</th><th className="px-5 py-3">Status</th></tr></thead><tbody className="divide-y">{[...allocations].sort((a,b)=>`${houses.find(h=>h.id===a.house_id)?.name||''}-${a.room_number}-${a.bed_space||''}`.localeCompare(`${houses.find(h=>h.id===b.house_id)?.name||''}-${b.room_number}-${b.bed_space||''}`,undefined,{numeric:true})).map(a=>{const s=studentMap.get(a.student_id);const house=houses.find(h=>h.id===a.house_id)?.name||s?.house||'Unassigned';return <tr key={a.student_id} className="hover:bg-slate-50"><td className="px-5 py-3 font-black text-slate-900">{s?.full_name||'Unknown student'}</td><td className="px-5 py-3 text-slate-500">{s?.admission_number||'—'}</td><td className="px-5 py-3 font-semibold">{house}</td><td className="px-5 py-3 font-bold text-blue-700">{a.room_number||'—'}</td><td className="px-5 py-3 font-bold text-indigo-700">{a.bed_space||'—'}</td><td className="px-5 py-3"><span className="rounded-full bg-emerald-50 px-2.5 py-1 text-[10px] font-black text-emerald-700">{a.status}</span></td></tr>})}</tbody></table></div>:<p className="p-10 text-center text-sm text-slate-500">No active room and bed allocations.</p>}
    </section>
