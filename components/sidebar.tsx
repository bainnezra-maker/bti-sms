...
type OwnerView = 'admin' | 'teacher' | 'housemaster';
...
export default function Sidebar(){
 ...
 useEffect(()=>{if(role==='owner'){const saved=window.localStorage.getItem('biritech-owner-view');if(saved==='admin'||saved==='teacher'||saved==='housemaster')setOwnerView(saved)}},[role]);
 ...
 const title=role==='owner'?`Owner · ${ownerView==='admin'?'Administration':ownerView==='teacher'?'Teacher Workspace':'Boarding'}`:role==='principal'?'Principal':role==='admin'?'Administrator':role==='teacher'?'Teacher Workspace':role==='Student'?'Student Portal':role==='housemaster'?'Housemaster / Housemistress':'BIRITECH SMS';
 const active=(href:string)=>pathname===href||(href!=='/'&&pathname.startsWith(href+'/'));
 const scopedHref=(href:string)=>{const gender=searchParams.get('gender');return effectiveRole==='housemaster'&&gender?`${href}?gender=${encodeURIComponent(gender)}`:href};
 async function logout(){setMobileOpen(false);await supabase.auth.signOut();router.replace('/login');router.refresh()}
 function switchOwnerView(view:OwnerView){setOwnerView(view);window.localStorage.setItem('biritech-owner-view',view);setMobileOpen(false);router.push(view==='admin'?'/':view==='teacher'?'/teacher':'/housemaster')}
 ...
 <div className="fixed left-0 right-0 top-0 z-40 flex h-16 items-center justify-between border-b bg-white/95 px-4 shadow-sm backdrop-blur lg:hidden">
  ...
  <Link href={home} className="flex items-center gap-2 font-black"><span className="flex h-10 w-10 items-center justify-center overflow-hidden rounded-full border-2 border-amber-400 bg-white"><img src="/biritech-logo.png" alt="Biriwa Technical Institute crest" className="h-full w-full scale-110 object-cover"/></span>BIRITECH SMS</Link>
 ...
 <aside className={`fixed bottom-0 left-0 top-0 z-50 flex w-72 max-w-[88vw] flex-col bg-white shadow-2xl transition-transform duration-300 lg:hidden ${mobileOpen?'translate-x-0':'-translate-x-full'}`}>
  ...
  {role==='owner'&&<div className="mx-4 mt-4 rounded-2xl border border-amber-200 bg-amber-50 p-3"><p className="mb-2 text-[10px] font-black uppercase tracking-widest text-amber-800">Switch portal</p><div className="grid grid-cols-3 gap-2"><button onClick={()=>switchOwnerView('admin')} className={`rounded-lg px-2 py-2 text-xs font-bold ${ownerView==='admin'?'bg-slate-900 text-white':'bg-white text-slate-700'}`}>Admin</button><button onClick={()=>switchOwnerView('teacher')} className={`rounded-lg px-2 py-2 text-xs font-bold ${ownerView==='teacher'?'bg-slate-900 text-white':'bg-white text-slate-700'}`}>Teacher</button><button onClick={()=>switchOwnerView('housemaster')} className={`rounded-lg px-2 py-2 text-xs font-bold ${ownerView==='housemaster'?'bg-slate-900 text-white':'bg-white text-slate-700'}`}>Boarding</button></div></div>}
 ...
 <aside className="fixed bottom-0 left-0 top-0 z-50 hidden w-64 flex-col overflow-y-auto border-r bg-white shadow-sm lg:flex">
  ...
  {role==='owner'&&<div className="mx-4 mt-4 rounded-2xl border border-amber-200 bg-amber-50 p-3"><p className="mb-2 text-[10px] font-black uppercase tracking-widest text-amber-800">Owner portal switcher</p><div className="grid grid-cols-3 gap-2"><button onClick={()=>switchOwnerView('admin')} className={`rounded-lg px-2 py-2 text-xs font-bold ${ownerView==='admin'?'bg-slate-900 text-white shadow':'bg-white text-slate-700'}`}>Admin</button><button onClick={()=>switchOwnerView('teacher')} className={`rounded-lg px-2 py-2 text-xs font-bold ${ownerView==='teacher'?'bg-slate-900 text-white shadow':'bg-white text-slate-700'}`}>Teacher</button><button onClick={()=>switchOwnerView('housemaster')} className={`rounded-lg px-2 py-2 text-xs font-bold ${ownerView==='housemaster'?'bg-slate-900 text-white shadow':'bg-white text-slate-700'}`}>Boarding</button></div></div>}
